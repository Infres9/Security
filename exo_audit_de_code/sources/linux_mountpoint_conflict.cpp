...
bool _is_share_available(std::string &unc) {
	...
	LOG(LOG_DEBUG, "callrpc to %s (program %d, proc %d, vers %d)", addr.c_str(), NFS_PROGRAM, NFSPROC_NULL, 3);
	int rpcRes = callrpc(addr.c_str(), NFS_PROGRAM, 3, NFSPROC_NULL, (xdrproc_t) xdr_void, (char*) NULL, (xdrproc_t) xdr_void, (char*) NULL);
	switch( rpcRes ) {
		case 0:
			LOG(LOG_DEBUG, "callrpc succes !", rpcRes);
			return true;
		case RPC_TIMEDOUT:
			LOG(LOG_DEBUG, "callrpc timeout !", rpcRes);
			return false;
		default:
			LOG(LOG_DEBUG, "callrpc failed (%d)", rpcRes);
			clnt_perrno((clnt_stat)rpcRes);
			return false;
	}
